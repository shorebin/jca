...
public class CompareTableCellRenderer extends JLabel implements TableCellRenderer {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4570025778499588114L;
	Configuration cfg;
	...
	public CompareTableCellRenderer(Configuration c) {
		this.cfg = c;
		initialize();
	}

	public CompareTableCellRenderer(String text) {
		super(text);
		initialize();
	}

	public CompareTableCellRenderer(String text, int horizontalAlignment) {
		super(text, horizontalAlignment);
		initialize();
	}

	public CompareTableCellRenderer(String text, Icon icon, int horizontalAlignment) {
		super(text, icon, horizontalAlignment);
		initialize();
	}

	public CompareTableCellRenderer(Icon image) {
		super(image);
		initialize();
	}

	public CompareTableCellRenderer(Icon image, int horizontalAlignment) {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
			int row, int column) {
		CompareCell cc = (CompareCell) value;
		...
		if ((cc != null) && (cc.threadDump[cc.index].javaStack[cc.threadIndex] != null)) {
			if (cc.index != 0) {
				if (cc.threadDump[(cc.index - 1)].threadHash.containsKey(cc.tid)) {
					int idx = ((Integer) cc.threadDump[(cc.index - 1)].threadHash.get(cc.tid)).intValue();
	...
	private void initialize() {
		try {
			setName("CompareTableCellRenderer");
			setOpaque(true);
			setText("");
			setSize(157, 14);
